function [ sigma ] = stdIm( im, type )
%STDIM Estimate the standard deviation of an image
%   sigma = stdIm(im)
%   sigma = stdIm(im, 'od2')
...
sumall = @(x) sum(x(:));
switch type
    case 'OD1' % ordinary difference of order 1
        [m, n] = size(im);
        sigma = sqrt((sumall(abs(diff(im, 1, 1)).^2)/(m-1)/n+...
            sumall(abs(diff(im, 1, 2)).^2)/(n-1)/m)/4);
    case 'OD2' % ordinary difference of order 2
        [m, n] = size(im);
        sigma = sqrt((sumall(abs(diff(im, 2, 1)).^2)/(m-2)/n+...
            sumall(abs(diff(im, 2, 2)).^2)/(n-2)/m)/12);
    case 'LTO' % by Liu, Tanaka and Okutomi (ICIP '12; IEEE TIP '13)
        sigma = NoiseLevel(im);
end
